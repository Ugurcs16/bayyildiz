  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl">
        <div className="flex snap-x snap-mandatory gap-3 overflow-x-auto overscroll-x-contain px-4 pb-2 pt-1 [scrollbar-width:thin] sm:gap-4 sm:px-6 md:pt-2 md:pb-3">
          {CATEGORIES.map((c) => (
            <Link
              key={c.href}
              href={c.href}
              className="relative h-24 w-36 shrink-0 snap-start overflow-hidden rounded-xl ring-1 ring-black/[0.06] transition-[transform,box-shadow] hover:shadow-md active:scale-[0.98] md:h-28 md:w-44"
            >
              <Image
                src={c.image}
                alt=""
              ...
              <div
                ...
                aria-hidden
              />
              <span className="absolute inset-0 flex items-center justify-center px-2 text-center text-sm font-semibold text-white drop-shadow-sm md:text-base">
                {c.title}
              </span>
            </Link>
          ))}
